...
// C/C++ source code generated on : Tue Dec 30 19:17:07 2025
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"test_position_control3\" **\n");
        ...
        rtmSetErrorStatus(test_position_control3_M, (NULL));
        test_position_control3_initialize();

        /* External mode */
        rtSetTFinalForExtMode(&rtmGetTFinal(test_position_control3_M));
        rtExtModeCheckInit(1);

        {
          boolean_T rtmStopReq = false;
          rtExtModeWaitForStartPkt(test_position_control3_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_position_control3_M, true);
          }
        }
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(test_position_control3_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(test_position_control3_M)
                == (NULL)) || rtmGetStopRequested(test_position_control3_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        test_position_control3_terminate();
        rtExtModeShutdown(1);
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(test_position_control3_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModePauseIfNeeded(test_position_control3_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_position_control3_M, true);
          }

          if (rtmGetStopRequested(test_position_control3_M) == true) {
    ...
    void NodeInterface::extmodeBackgroundTask(void)
    {
      while (mRunModel) {
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModeOneStep(test_position_control3_M->extModeInfo, 1,
                           &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_position_control3_M, true);
          }
