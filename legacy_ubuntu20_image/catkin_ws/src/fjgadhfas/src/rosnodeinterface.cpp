...
// C/C++ source code generated on : Thu Jan 15 18:04:54 2026
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"fjgadhfas\" **\n");
        ...
        rtmSetErrorStatus(fjgadhfas_M, (NULL));
        fjgadhfas_initialize();

        /* External mode */
        rtSetTFinalForExtMode(&rtmGetTFinal(fjgadhfas_M));
        rtExtModeCheckInit(1);

        {
          boolean_T rtmStopReq = false;
          rtExtModeWaitForStartPkt(fjgadhfas_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(fjgadhfas_M, true);
          }
        }
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(fjgadhfas_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(fjgadhfas_M)
                == (NULL)) || rtmGetStopRequested(fjgadhfas_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(fjgadhfas_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModePauseIfNeeded(fjgadhfas_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(fjgadhfas_M, true);
          }

          if (rtmGetStopRequested(fjgadhfas_M) == true) {
    ...
    void NodeInterface::extmodeBackgroundTask(void)
    {
      while (mRunModel) {
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModeOneStep(fjgadhfas_M->extModeInfo, 1,
                           &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(fjgadhfas_M, true);
          }
